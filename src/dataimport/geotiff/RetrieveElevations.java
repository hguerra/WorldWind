...
public class RetrieveElevations extends ApplicationTemplate {
	...
	public static class AppFrame extends ApplicationTemplate.AppFrame {
		...
		public AppFrame() {
			// We add our own LayerPanel, but keep the StatusBar from
			// ApplicationTemplate.
		...
		protected void makeComponents() {
			((Component) this.getWwd()).setPreferredSize(new Dimension(1024,
					768));
			...
			JPanel panel = new JPanel(new BorderLayout());
			{
				...
				JPanel vePanel = new JPanel(new BorderLayout(0, 5));
				{
					...
					int MIN_VE = 1;
					int MAX_VE = 8;
					int curVe = (int) this.getWwd().getSceneController()
							.getVerticalExaggeration();
					curVe = curVe < MIN_VE ? MIN_VE : (curVe > MAX_VE ? MAX_VE
							: curVe);
					...
					slider.addChangeListener(new ChangeListener() {
						public void stateChanged(ChangeEvent e) {
							...
							ActionEvent ae = new ActionEvent(ve, 0,
	...
	public static class ElevationsDemoController implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			} else if (ACTION_COMMAND_BUTTON2.equalsIgnoreCase(e
					.getActionCommand())) {
				this.doActionOnButton2();
			} else if (ACTION_COMMAND_BUTTON3.equalsIgnoreCase(e
					.getActionCommand())) {
				this.doActionOnButton3();
			} else if (ACTION_COMMAND_VERTICAL_EXAGGERATION.equalsIgnoreCase(e
					.getActionCommand())) {
		...
		public void doActionOnButton1() {
			...
			Position matterhorn = new Position(LatLon.fromDegrees(
					45.9763888888889d, 7.65833333333333d), 0d);
		...
		public void doActionOnButton2() {
			...
			double resolutionAchieved = this.wwd
					.getModel()
					.getGlobe()
					.getElevationModel()
		...
		public void doActionOnButton3() {
			...
			ElevationModel model = this.wwd.getModel().getGlobe()
					.getElevationModel();

			StringBuffer sb = new StringBuffer();
			for (LatLon ll : latlons) {
				double e = model.getElevation(ll.getLatitude(),
						ll.getLongitude());
				sb.append("\n").append(e);
			}
